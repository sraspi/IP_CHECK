...
data = "/home/pi/stta/data/" + "Koji.txt"
f = open(data,  "w")
f.write("Datum/Zeit,  Koji-Temp.[°C],   cpu[%]" + '\n')
f.close()
...
fig.canvas.manager.set_window_title(" Koji-Temp")
...
(vol_line,) = ax.plot(x, y, label=" Koji-Temp", color="#00549F", marker=".") #00549F is the RWTH blue color
ax.set_ylabel("Koji-Temp", color="#00549F")
...
#ax2.set_ylabel("TGZ 18 CO2-Volumenstrom [ml/h]", color="#CC071E")
#plt.ylim(22, 35)#commented out because it overrides dynamic axes scaling
plt.title(" Koji-Temp: " + "\n" + str(T1) + " °C", fontsize=25) # To display 0ml initially. Will be updated in an event-driven manner (see on_trigger)
...
try:
    while True:
        ...
        plt.title(timestr + "  Koji-Temp: " + "\n" + str(T1) + " °C", fontsize=25)#Diagrammtitel
